...
function updateComment(url, request) {
  ...
  if (!id || !requestComment || !request.body || requestComment.body === "") {
    response.status = 400;
...
function deleteComment(url, request){
  ...
  if(!savedComment){response.status = 404;return response;}
  
  if (savedComment) {
    database.comments[id] = null;
    let user = database.users[savedComment.username];
      user.commentIds.splice(user.commentIds.indexOf(id),1);
    let article = database.articles[savedComment.articleId];
      article.commentIds.splice(article.commentIds.indexOf(id),1);
    response.body = {comment: savedComment};
    response.status = 204;
  }
...
function upvoteComment(url, request) {
  ...
  else if(savedComment && database.users[username]) {
...
function saveDatabase(){
  yaml('db.yml', database, function(err) {
    console.log(err);
  });
}

function loadDatabase(){
  readYaml('db.yml', function(err, data) {
    if (err) throw err;
    console.log(data);
  });
}
